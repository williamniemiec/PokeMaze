...
bool Collisions::has_collision_plane_plane(SceneObject* obj1, SceneObject* obj2)
{
    ...
    //glm::vec3 bbox_min_obj1 = obj1.get_world_bounding_box();

    return  (bbox_world_obj1->get_world_min_x() <= bbox_world_obj2->get_world_max_x() && bbox_world_obj1->get_world_max_x() >= bbox_world_obj2->get_world_min_x())
            && (bbox_world_obj1->get_world_min_y() <= bbox_world_obj2->get_world_max_y() && bbox_world_obj1->get_world_max_y() >= bbox_world_obj2->get_world_min_y())
            && (bbox_world_obj1->get_world_min_z() <= bbox_world_obj2->get_world_max_z() && bbox_world_obj1->get_world_max_z() >= bbox_world_obj2->get_world_min_z());
}
...
bool Collisions::has_collision_point_plane(glm::vec4 point, SceneObject* plane)
{
    ...
    if (bbox_world->get_world_min_x() == bbox_world->get_world_max_x())
    {
        if (bbox_world->get_world_min_x() >= 0)
        {
            has_colision =  (point.x >= bbox_world->get_world_min_x())
                    && (point.y >= bbox_world->get_world_min_y() && point.y <= bbox_world->get_world_max_y())
                    && (point.z >= bbox_world->get_world_min_z() && point.z <= bbox_world->get_world_max_z());
        }
        else
        {
            has_colision =  (point.x <= bbox_world->get_world_min_x())
    ...
    else if (bbox_world->get_world_min_y() == bbox_world->get_world_max_y())
    {
        ...
        {
            has_colision =  (point.x >= bbox_world->get_world_min_x() && point.x <= bbox_world->get_world_max_x())
...
bool Collisions::has_collision_sphere_plane(SceneObject* sphere, SceneObject* plane)
{
    ...
    double x = std::max(bbox_world->get_world_min_x(), std::min(sphere->get_position_x(), bbox_world->get_world_max_x()));
    double y = std::max(bbox_world->get_world_min_y(), std::min(sphere->get_position_y(), bbox_world->get_world_max_y()));
    double z = std::max(bbox_world->get_world_min_z(), std::min(sphere->get_position_z(), bbox_world->get_world_max_z()));

    glm::vec3 position = {x, y, z};
    glm::vec3 sphere_position = {sphere->get_position_x(), sphere->get_position_y(), sphere->get_position_z()};
